    ...
    NotificationServer {
        ...
        onNotification: (n) => {
            ...
            const forceIconApps = [
                ...
                "whatsapp", "whatsapp-desktop", "whatsapp-for-linux",
            ...
            // Icon theme based resolution (rofi/wofi tarzı)
            var appLower = (n.desktopEntry || "").toLowerCase();
            var appNameLower = (n.appName || "").toLowerCase().replace(/\s+/g, "-");
            var knownIcons = {
                "telegram": "telegram",
            ...
            if (knownIcons[appLower]) {
                finalImage = "image://icon/" + knownIcons[appLower];
            } else if (knownIcons[appNameLower]) {
                finalImage = "image://icon/" + knownIcons[appNameLower];
            }
            // 分支 A: 允许显示图片 (截图、非黑名单App)
            else if (!shouldForceIcon && n.image && (n.image.startsWith("/") || n.image.startsWith("file://"))) {
