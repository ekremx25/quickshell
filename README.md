...
| [Hyprland](https://github.com/hyprwm/Hyprland) | Full | Socket2 event stream; `.conf` and Lua config providers supported; Night Light requires `hyprsunset` |
...
Quickshell supports modern Hyprland Lua configs as well as classic `.conf` files. The Hyprland settings pages detect the active config provider and apply monitor, mouse, workspace, gaps, borders, rounding, shadow, HDR, and ICC profile settings through Lua-aware helper scripts when Hyprland is running with `configProvider: lua`.

This means a setup based on `~/.config/hypr/hyprland.lua` can still be managed from the Quickshell settings dashboard without converting back to `.conf`.
